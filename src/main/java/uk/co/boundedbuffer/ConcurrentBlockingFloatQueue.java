...
/**
 * A low latency, lock free, primitive bounded blocking queue backed by an float[]. This class mimics the
 * interface of {@linkplain java.util.concurrent.BlockingQueue BlockingQueue}, however works with primitive
 * floats rather than {@link Object}s, so is unable to actually implement the BlockingQueue . <p> This class
 ...
 * <tt>uk.co.boundedbuffer.ConcurrentBlockingFloatQueue</tt> methods are thread-safe when used with a single
 * producer and single consumer, internal atomicity is achieved using lock free strategies, such as sping
 * locks. <p> <p>Like a <tt>BlockingQueue</tt>, the uk.co.boundedbuffer.ConcurrentBlockingFloatQueue does
...
public class ConcurrentBlockingFloatQueue extends AbstractBlockingQueue {
     ...
     */
    public ConcurrentBlockingFloatQueue() {
     ...
     */
    public ConcurrentBlockingFloatQueue(int size) {
        super(size);
    }
     ...
     * queue, it is generally preferable to use {@link #offer(float) offer}.
     *
     ...
     * #offer(float)}, {@link #put(float)},  {@link #drainTo(float[], int)}}
     *
