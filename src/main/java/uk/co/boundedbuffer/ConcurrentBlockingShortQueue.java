...
/**
 * A low latency, lock free, primitive bounded blocking queue backed by an short[]. This class mimics the
 * interface of {@linkplain java.util.concurrent.BlockingQueue BlockingQueue}, however works with primitive
 * shorts rather than {@link Object}s, so is unable to actually implement the BlockingQueue . <p> This class
 ...
 * put(e)}</td> <td>{@link #offer(short, long, java.util.concurrent.TimeUnit) offer(e, time, unit)}</td> </tr>
 ...
 * </table> <p> <p> <p>A <tt>uk.co.boundedbuffer.ConcurrentBlockingShortQueue</tt> is capacity bounded. At any
 ...
 * <tt>uk.co.boundedbuffer.ConcurrentBlockingShortQueue</tt> methods are thread-safe when used with a single
 * producer and single consumer, internal atomicity is achieved using lock free strategies, such as sping
 * locks. <p> <p>Like a <tt>BlockingQueue</tt>, the uk.co.boundedbuffer.ConcurrentBlockingShortQueue does
...
public class ConcurrentBlockingShortQueue extends AbstractBlockingQueue {
    ...
    private final short[] data = new short[capacity];
     ...
     */
    public ConcurrentBlockingShortQueue(int capacity) {
        super(capacity);
    }
     ...
     * queue, it is generally preferable to use {@link #offer(short) offer}.
     *
     ...
     * #take()}. {@link #offer(short)}, {@link #put(short)},  {@link #drainTo(short[], int)}}
     *
