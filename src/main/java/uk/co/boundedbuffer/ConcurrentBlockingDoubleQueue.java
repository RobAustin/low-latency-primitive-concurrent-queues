...
/**
 * A low latency, lock free, primitive bounded blocking queue backed by an double[]. This class mimics the
 * interface of {@linkplain java.util.concurrent.BlockingQueue BlockingQueue}, however works with primitive
 * doubles rather than {@link Object}s, so is unable to actually implement the BlockingQueue . <p> This class
 ...
 * is a classic &quot;bounded buffer&quot;, in which a fixed-capacityd array holds elements inserted by
 * producers and extracted by consumers.  Once created, the capacity cannot be changed.  Attempts to {@link
 * #put put(e)} an element into a full queue will result in the operation blocking; attempts to {@link #take
 * take()} an element from an empty queue will similarly block. <p> <p>Due to the lock free nature of its
 ...
 * put(e)}</td> <td>{@link #offer(double, long, java.util.concurrent.TimeUnit) offer(e, time, unit)}</td>
 * </tr> <tr> <td><b>Remove</b></td> <td>{@link #poll poll()}</td> <td>not applicable</td> <td>{@link #take
 ...
 * </table> <p> <p> <p>A <tt>uk.co.boundedbuffer.ConcurrentBlockingDoubleQueue</tt> is capacity bounded. At
 * any given time it may have a <tt>remainingCapacity</tt> beyond which no additional elements can be
 * <tt>put</tt> without blocking. <p> <p> It is not possible to remove an arbitrary element from a queue using
 * <tt>remove(x)</tt>. As this operation would not performed very efficiently. <p> <p>All of
 * <tt>uk.co.boundedbuffer.ConcurrentBlockingDoubleQueue</tt> methods are thread-safe when used with a single
 * producer and single consumer, internal atomicity is achieved using lock free strategies, such as sping
 * locks. <p> <p>Like a <tt>BlockingQueue</tt>, the uk.co.boundedbuffer.ConcurrentBlockingDoubleQueue does
 * <em>not</em> intrinsically support any kind of &quot;close&quot; or &quot;shutdown&quot; operation to
 * indicate that no more items will be added.  The needs and usage of such features tend to be
 * implementation-dependent. For example, a common tactic is for producers to insert special
 * <em>end-of-stream</em> or <em>poison</em> objects, that are interpreted accordingly when taken by
 * consumers. <p> <p> Usage example, based on a typical producer-consumer scenario. Note that a
 * <tt>BlockingQueue</tt> can ONLY be used with a single producers and single consumer thead.
 * <pre>
 *
 ...
 * thread. <p> <p> <p> Copyright 2014 Rob Austin <p> Licensed under the Apache License, Version 2.0 (the
...
public class ConcurrentBlockingDoubleQueue extends AbstractBlockingQueue {
     ...
     * queue, it is generally preferable to use {@link #offer(double) offer}.
     *
    ...
    public double peek(long timeout, TimeUnit unit)
     ...
     * @throws InterruptedException if interrupted while waiting
     */
    public boolean offer(double value, long timeout, TimeUnit unit)
     ...
     * @throws java.util.concurrent.TimeoutException if timeout time is exceeded
     */
    public double poll(long timeout, TimeUnit unit)
            throws InterruptedException, TimeoutException {
     ...
     * smaller than the number of elements then the number of elements read will equal the capacity of the
     * array. This operation may be more efficient than repeatedly polling this queue.  A failure encountered
     * while attempting to add elements to collection <tt>c</tt> may result in elements being in neither,
     * either or both collections when the associated exception is thrown.  Further, the behavior of this
     * operation is undefined if the following methods are called during progress of this operation {@link
     * #take()}. {@link #offer(double)}, {@link #put(double)},  {@link #drainTo(double[], int)}}
     *
     ...
     *                                       specified collection
     */
    int drainTo(double[] target, int maxElements) {
