 ...
 * put(e)}</td> <td>{@link #offer(long, long, java.util.concurrent.TimeUnit) offer(e, time, unit)}</td> </tr>
 * <tr> <td><b>Remove</b></td> <td>{@link #poll poll()}</td> <td>not applicable</td> <td>{@link #take
 * take()}</td> <td>{@link #poll(long, java.util.concurrent.TimeUnit) poll(time, unit)}</td> </tr> <tr>
 ...
 * </table> <p> <p> <p>A <tt>uk.co.boundedbuffer.ConcurrentBlockingLongQueue</tt> is capacity bounded. At any
 ...
 * <tt>uk.co.boundedbuffer.ConcurrentBlockingLongQueue</tt> methods are thread-safe when used with a single
 * producer and single consumer, internal atomicity is achieved using lock free strategies, such as sping
 * locks. <p> <p>Like a <tt>BlockingQueue</tt>, the uk.co.boundedbuffer.ConcurrentBlockingLongQueue does
...
public class ConcurrentBlockingLongQueue extends AbstractBlockingQueue {
     ...
     */
    public ConcurrentBlockingLongQueue(int capacity) {
        super(capacity);
    }
     ...
     * queue, it is generally preferable to use {@link #offer(long) offer}.
     *
     ...
     * #take()}. {@link #offer(long)}, {@link #put(long)},  {@link #drainTo(long[], int)}}
     *
